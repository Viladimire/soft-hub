...
const request = async <T,>(path: string, init?: RequestInit) => {
  ...
  if (!response.ok) {
    ...
    const message = typeof payload?.message === "string" ? payload.message : "تعذر تنفيذ العملية";
...
export const RequestsAdminPanel = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      pushNotification("success", "تم حفظ الطلب");
      closeForm();
      await syncDataset();
    } catch (err) {
      pushNotification("error", err instanceof Error ? err.message : "فشل حفظ الطلب");
      if ((err as RequestError).status === 401) {
        router.refresh();
      }
    } finally {
  ...
  const handleDelete = async (item: SoftwareRequest) => {
    if (!confirm(`هل تريد حذف الطلب: ${item.name}؟`)) {
      return;
    }
    ...
    try {
      ...
      pushNotification("success", "تم حذف الطلب");
      await syncDataset();
    } catch (err) {
      pushNotification("error", err instanceof Error ? err.message : "فشل حذف الطلب");
      if ((err as RequestError).status === 401) {
        router.refresh();
      }
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-white">طلبات إضافة برامج</h2>
        <p className="text-sm text-neutral-400">إدارة طلبات إضافة برامج عبر GitHub dataset (CRUD كامل).</p>
      </div>
      ...
      {isFormOpen ? (
        <Card className="border-white/10 bg-neutral-950/60">
          <CardHeader>
            <CardTitle className="text-base text-white">{formState.id ? "تعديل طلب" : "إضافة طلب"}</CardTitle>
          </CardHeader>
          <CardContent>
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div className="space-y-2">
                <label className="block text-xs text-neutral-400">اسم البرنامج</label>
                <Input
                  value={formState.name}
                  onChange={(event) => handleFormChange("name", event.target.value)}
                  className="bg-neutral-900/80 text-neutral-100"
                  placeholder="مثال: Notion"
                />
              </div>

              <div className="space-y-2">
                <label className="block text-xs text-neutral-400">موقع البرنامج (اختياري)</label>
                <Input
              ...
              <div className="space-y-2">
                <label className="block text-xs text-neutral-400">الحالة</label>
                <select
              ...
              <div className="space-y-2">
                <label className="block text-xs text-neutral-400">ملاحظات</label>
                <Textarea
                  value={formState.notes}
                  onChange={(event) => handleFormChange("notes", event.target.value)}
                  className="min-h-[120px] bg-neutral-900/80 text-neutral-100"
                  placeholder="أي تفاصيل إضافية عن سبب الطلب / المصدر / البدائل..."
                />
              </div>
      ...
      <Card className="border-white/10 bg-neutral-950/60">
        <CardHeader>
          <CardTitle className="text-base text-white">قائمة الطلبات</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3 text-sm text-neutral-300">
          {loading ? <p className="text-neutral-400">جارٍ التحميل...</p> : null}
          {!loading && !dataset.length ? <p className="text-neutral-400">لا توجد طلبات بعد.</p> : null}
